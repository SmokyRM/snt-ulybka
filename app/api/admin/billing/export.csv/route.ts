...
import { unauthorized, forbidden, badRequest, fail, serverError } from "@/lib/api/respond";
...
export async function GET(request: Request) {
  try {
    ...
    if (!periodId) return badRequest(request, "periodId required");

    const period = listAccrualPeriods().find((p) => p.id === periodId);
    if (!period) return fail(request, "not_found", "period not found", 404);
