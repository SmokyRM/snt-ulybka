...
const SEGMENTS: Segment[] = ["all", "has_phone", "no_phone", "overdue", "large_debt"];
...
export async function GET(request: Request) {
  ...
  const segment: Segment = SEGMENTS.includes(segmentRaw as Segment) ? (segmentRaw as Segment) : "all";
  const minDebt = parseNum(searchParams.get("minDebt"), 0);
  const minOverdueDays = parseNum(searchParams.get("minOverdueDays"), 0);

  const allPeriods = listUnifiedBillingPeriods();
  const periods = allPeriods.filter((p) => statusOk(p.status));

  if (periods.length === 0) {
      const empty: DebtorsResponse = {
        rows: [],
        totals: { count: 0, debtTotal: 0, withPhone: 0, withoutPhone: 0 },
      };
  ...
  function getOrCreate(key: string, fullName: string, phone: string | null): Agg {
    let v = agg.get(key);
    if (!v) {
      v = { fullName, phone, plotIds: new Set(), debtTotal: 0, minPeriodTo: null };
      agg.set(key, v);
    }
    return v;
  }
  ...
  for (const selectedPeriod of periods) {
    ...
    for (const plot of plots) {
      ...
      const v = getOrCreate(key, fullName, phone);
  ...
  let rows: DebtorRow[] = Array.from(agg.entries()).map(([personId, v]) => ({
    personId,
    fullName: v.fullName,
    phone: (v.phone ?? "").trim() || "",
  ...
  switch (segment) {
    case "has_phone":
      rows = rows.filter((r) => hasPhone(r.phone));
      break;
    case "no_phone":
      rows = rows.filter((r) => !hasPhone(r.phone));
      break;
    case "overdue":
      rows = rows.filter((r) => r.debtTotal > 0 && r.overdueDays > minOverdueDays);
      break;
    case "large_debt":
      rows = rows.filter((r) => r.debtTotal >= minDebt);
      break;
    case "all":
    default:
      break;
  ...
  const debtTotalSum = rows.reduce((s, r) => s + r.debtTotal, 0);
  const withPhone = rows.filter((r) => hasPhone(r.phone)).length;
  const withoutPhone = rows.filter((r) => !hasPhone(r.phone)).length;

  const totals: DebtorsTotals = {
    count: rows.length,
    debtTotal: debtTotalSum,
    withPhone,
    withoutPhone,
  };

    const response: DebtorsResponse = { rows, totals };
