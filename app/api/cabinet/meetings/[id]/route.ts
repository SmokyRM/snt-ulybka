...
import { getMeetingById, listAgendaItems, listMaterials, listQuestions, hasPgConnection } from "@/lib/meetings.pg";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const materials = await listMaterials(id, "residents");
    const questions = await listQuestions(id, false);
    const votes = await listMeetingVotes(id);
    return ok(request, { meeting, agenda, materials, questions, votes });
  } catch (error) {
    return serverError(request, "Ошибка загрузки собрания", error);
  }
}
