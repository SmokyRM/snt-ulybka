...
export default async function LoginPage({
  ...
  const devLoginCode = isDev && (process.env.DEV_LOGIN_CODE || process.env.MASTER_CODE)
    ? (process.env.DEV_LOGIN_CODE || process.env.MASTER_CODE)
    : null;

  return (
    <main className="min-h-screen bg-[#F8F1E9] px-4 py-12 text-zinc-900 sm:px-6">
      <div className="mx-auto w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm">
        <LoginForm nextParam={rawNext} devLoginCode={devLoginCode} showDevHints={isDev} />
