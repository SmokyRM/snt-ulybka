...
export default async function OfficeTemplateEditPage({ params }: Props) {
  const { id } = await params;
  const user = await getEffectiveSessionUser();
  if (!user) redirect(`/staff/login?next=/office/templates/${id}/edit`);
  ...
  try {
    ...
    if (error instanceof Error && error.message === "UNAUTHORIZED") {
      redirect(`/staff/login?next=/office/templates/${id}/edit`);
    }
  ...
  async function save(formData: FormData) {
    ...
    if (!session) redirect(`/staff/login?next=/office/templates/${id}/edit`);
  ...
  return (
    <div className="space-y-4" data-testid="template-editor">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-900">Редактировать шаблон</h1>
          <p className="text-sm text-zinc-600">Обновите текст шаблона</p>
        </div>
        <AppLink
      ...
      <form action={save} className="space-y-3 rounded-2xl border border-zinc-200 bg-white px-4 py-5 shadow-sm" data-testid="template-editor">
