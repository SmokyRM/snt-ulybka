...
export default function ImportPaymentsClient() {
  ...
  const runImport = async (mode: "preview" | "apply") => {
    ...
    try {
      ...
      const response = await fetch("/api/office/billing/import-payments", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="space-y-4" data-testid="office-billing-import-root">
      <div>
        <h1 className="text-2xl font-semibold text-zinc-900">Импорт платежей</h1>
        <p className="text-sm text-zinc-600">Загрузите CSV файл с платежами для проверки.</p>
      </div>
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
        <label className="block text-sm font-semibold text-zinc-700">
          ...
          <input
            ...
            data-testid="office-billing-import-upload"
          />
        </label>
        {showReason && (
          <div className="mt-3">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button
            ...
            data-testid="office-billing-import-preview"
          >
          ...
          <button
            ...
            data-testid="office-billing-import-submit"
          >
            Импортировать
          </button>
          <a
      ...
      {loading ? <OfficeLoadingState message="Обрабатываем CSV..." /> : null}
      {error ? <OfficeErrorState message={error} /> : null}

      {preview ? (
        <ImportResultCard title="Предпросмотр" result={preview} testId="office-billing-import-preview-result" />
      ) : !loading && !error ? (
        <OfficeEmptyState message="Загрузите CSV файл, чтобы увидеть предпросмотр." />
      ) : null}

      {result ? (
        <ImportResultCard title="Результат импорта" result={result} testId="office-billing-import-result" />
      ) : null}
    </div>
  );
}
